...
GooglePlayServices::Availability GooglePlayServices::getAvailability() {
  ...
  auto availablity =
      ::google_play_services::CheckAvailability(env, activity.object());
  qDebug() << "GooglePlayServices::getAvailability result :" << availablity
           << " (0 is kAvailabilityAvailable)";
...
bool GooglePlayServices::available() {
  ...
  auto availablity =
      ::google_play_services::CheckAvailability(env, activity.object());
  qDebug() << "GooglePlayServices::available() result :" << availablity
           << " (0 is kAvailabilityAvailable)";
...
#else
void PlatformUtils::getNativeWindow() { qDebug() << "Not available"; }
#endif
